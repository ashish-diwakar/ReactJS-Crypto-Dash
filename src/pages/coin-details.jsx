...
const CoinDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchCoin = async () => {
            try {
                ...
                if (!res.ok) throw Error('Failed to load coin data');
    ...
    return ( 
        <div className="coin-details-container">
            ...
            <h1 className="coins-details-title">
            ...
            {!loading && !error && (
                <>
                <img src={coin.image.large} alt={coin.name} className="coin-details-image"></img>
                {!showFullDetails && (
                    <>
                    <p>
                        {coin.description.en.split('. ')[0]}.
                        <button className="more-btn" onClick={() => setShowFullDetails(!showFullDetails)}>Show more..</button>
                    </p>
                    </>
                )}
                {showFullDetails && (
                    <p>
                        {coin.description.en}
                        <button className="more-btn" onClick={() => setShowFullDetails(!showFullDetails)}>Hide full details..</button>
                    </p>
                )}
                ...
                <div className="coin-details-links">
                    {coin.links.homepage[0] && (
                        <p>
                            ⚽{' '}
                            <a href={coin.links.homepage[0]} target="_blank" rel='noopner noreferrer'>
                                Website
                            </a>
                        </p>
                    )}
                    {coin.links.blockchain_site[0] && (
                        <p>
                            ❈{' '}
                            <a href={coin.links.blockchain_site[0]} target="_blank" rel='noopner noreferrer'>
                                Blockchain Explorer
                            </a>
                        </p>
                    )}
